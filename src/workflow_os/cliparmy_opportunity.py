"""Fail-closed bridge from Clip Army public discovery into Opportunity Manager input.

Public Clip Army campaign discovery is useful for ranking opportunities, but it does
...
class TrustedClipArmyCampaignEvidence:
...
def _verified_discovery(record: DiscoveryRecord) -> None:
    ...
    if record.source_platform != "cliparmy":
        raise ValueError("discovery record is not a Clip Army record")
    if record.fields.get("machine_submission_verified") is not False:
        raise ValueError("Clip Army public discovery must not assert machine submission")
    if record.fields.get("zero_touch_execution_enabled") is not False:
        raise ValueError("Clip Army public discovery must keep zero-touch execution disabled")
    if record.fields.get("execution_block_reason") != "official_creator_machine_submission_interface_not_verified":
        raise ValueError("Clip Army execution block reason is missing or inconsistent")
    if not _SHA256_RE.fullmatch(record.raw_evidence_sha256):
        raise ValueError("discovery evidence digest is malformed")


def build_cliparmy_opportunity(
    ...
    if not isinstance(evidence, TrustedClipArmyCampaignEvidence):
        raise TypeError("evidence must be TrustedClipArmyCampaignEvidence")
    ...
    if not all(flag is True for flag in required_flags):
        raise ValueError("Clip Army campaign evidence is incomplete")
